...
function Header() {
  ...
  useEffect(() => {
    if (authStatus) {
      const arr = Array.from(userData.name);
      setUserLogo(arr[0]);
    }
      ...
      <nav className="bg-white dark:bg-gray-900 w-full start-0 border-b-2 border-gray-200 dark:border-gray-600">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          ...
          <div className="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse gap-2">
            {authStatus ? (
              <>
              <Link to={`/:${userData.name || 'Anom'}/dashboard`}>
                <div
                  className={`w-10 h-10 cursor-pointer rounded-full flex items-center justify-center text-white text-2xl`}
